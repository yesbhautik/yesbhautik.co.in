...
function Post(props) {
  ...
  const title = `${props.title} // Yesbhautik`
  const description = props.description || ''
  const url = `https://yesbhautik.co.in/${props.slug}`
  const date = new Date(props.date).toISOString()
  const image = props.image
    ? `https://yesbhautik.co.in${props.image}`
    : 'https://yesbhautik.co.in/static/images/home-opt.jpg'
  ...
  return (
    <>
      ...
      <ArticleJsonLd
        authorName="Bhautik Bavadiya [Yesbhautik]"
        type="Blog"
        url={url}
        title={title}
        images={[image]}
...
export async function getStaticProps({ params }) {
  try {
    ...
    const base = isProd
      ? 'https://yesbhautik.co.in'
      : 'http://yesbhautik.co.in:3000'
